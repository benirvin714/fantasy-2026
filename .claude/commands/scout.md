...
## Procedure
1. **Get the batch + current flags:** run `node scripts/validate-scouting.mjs --worklist 10` (use the requested batch size). It prints coverage, the next N unscouted skill players **in ADP order**, and any current flags (dead source URLs, stale briefs). Your batch = that worklist. If it also flags a **dead source** or a **stale brief** on an already-scouted player, fold a fix for that player into this batch too (re-scout him).
