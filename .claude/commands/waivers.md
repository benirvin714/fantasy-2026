...
7. **Output**: ranked table + bid plan. End with total FAAB committed if all claims hit, and what remains.
   ...
   - **Sort order (fixed, not a judgment call).** Order `targets` by these keys in turn, then number `rank` 1..N in that order:
     1. **verdict**: PURSUE, then WATCH, then AVOID.
     2. **worth**, highest first. In `pit` and `clash`, where worth is unpriced, skip this key.
     3. **confidence**: high, then med, then low.
     4. **pressure**: high, then medium, then low (a contested add needs the earlier look).
     5. Anything still tied: alphabetical by player name, so a re-run on unchanged inputs produces the same order.
