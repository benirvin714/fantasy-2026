...
2. **My lineup**: `get_league_rosters` roster 10 (HBGBs). This command is **HBGBs-only for now** — for another league pass its id from `scripts/lib/leagues.mjs` and use that league's roster (**1** in The Panther Pit, **11** in Couples Clash), reading its delta file alongside `league-profile.md`: `league-profile-pit.md` for the 12-team depth delta, `league-profile-clash.md` for the 14-team depth delta **and full PPR**. The full-PPR difference is not cosmetic in a start/sit: at 1.0 per reception a high-target, low-aDOT option can out-score a boom/bust deep threat it would lose to in the other two leagues, so re-rank rather than reusing an HBGBs call — current starters vs bench, plus IR. `get_matchups` for this week → my opponent's roster; note their projected strengths for risk-calibration (favored → prefer floor; underdog → prefer ceiling).
