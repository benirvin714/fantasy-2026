...
- `data/site/team-environment.json` — all 32 offenses by projected points scored, with a league-format starter core and pass share. Powers the **Offenses** block under the target rail. Rebuild: `node scripts/build-team-environment.mjs` (`--offline` skips the 5MB ESPN PDF fetch and uses the committed snapshot `data/raw/clay-team-projections.json`). Context only, never an input to draft-$ or edge. Spec: `plans/valuation-and-scouting.md` §1.19.
