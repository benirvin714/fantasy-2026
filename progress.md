...
- **Drop-down is prioritized, not exhaustive** (2026-07-27): it opens on the four reads that drive a pick — **Why here → Scouting → Historical usage (with trends) → Boris Chen** — and puts the rest (Value & $ engine, Ceiling, Availability, Situation & risk, "How this value is built") behind compact disclosure buttons, all collapsed by default. ~51% shorter on open (769px vs 1577px). Nothing was removed and no black box: every field is one click away, section state is board-wide and survives repaints, and it resets compact on reload.
