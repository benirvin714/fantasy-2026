...
- **Target rail** (2026-07-28): a sticky shortlist column beside the board. ☆ on any row adds a player; the rail sorts by ADP and shows, per target, his ADP + its round and **how far away he is right now** — `29 past ADP` (green: the market is letting him fall), `on the clock`, or `45 picks · 4.5 rd`. Distance counts from a live pick number derived from your drafted marks (`taken.size + 1`), and the rail states that derivation rather than hiding it. Targets **drop off when marked drafted** by filtering, not deleting, so a mis-click is reversible and a `N gone:` line still tells you who got sniped. Rail is independent of the board's filters/sort/view; below 1040px it stacks above the board.
