...
| `plans/` | Design docs, now built and kept as the design of record. [valuation-and-scouting.md](plans/valuation-and-scouting.md): the 3-layer valuation model (asset, scarcity, auction-$, market edge) and the `scouting_brief` evidence layer that feeds the confidence band via `role_stability`, never value. |
