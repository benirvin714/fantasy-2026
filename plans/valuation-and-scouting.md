...
### 2.7 Pre-draft sourcing bar for the full draftable board, ADP 1-180 (2026-08-11)
...
- **Beat-first bar (every drafted player).** `/scout` and the daily SKILL now require at least one beat, coach, or camp-report source for a brief, or an explicit sentiment-only note in the rationale. Analyst blurbs are secondary, never the basis. A late player with no findable beat source is marked sentiment-only, not dressed up.
